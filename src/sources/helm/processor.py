...
# Import functions from local downloader
from src.sources.helm.downloader import (
...
def helm_main_func(csv_file: str, output_dir: Path, benchmark: str, adapter_method: str = None, 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="python main.py helm",
        description="Process HELM data from download to conversion"
    )
    ...
    helm_main_func(csv_file=csv_to_process_str, output_dir=output_dir_path, benchmark=args.benchmark,
...
def main():
    """Main entry point that can be called by the unified processor."""
    if __name__ == "__main__":
        # When called directly, use command line arguments
        import argparse
        ...
        if not csv_to_process.exists() or args.overwrite:
            ...
            try:
                # Import web_scraper lazily to avoid requiring Playwright at module import time
                from .web_scraper import main as create_csv_main
                asyncio.run(create_csv_main(benchmark=args.benchmark, output_dir=str(BENCHMARK_CSVS_DIR)))
                log_success(f"Successfully created CSV: {csv_to_process}", "✅")
            except Exception as e:
                log_error(f"Failed to create CSV for benchmark '{args.benchmark}': {e}", "❌")
        ...
        # When called from the unified interface, parse sys.argv manually
        # This allows the unified main.py to pass arguments properly
        import argparse
        parser = argparse.ArgumentParser(
            prog="python main.py helm",
            description="Process HELM data from download to conversion"
